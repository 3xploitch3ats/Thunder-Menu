    ...
    HDC hdcSrc, int xSrc, int ySrc, DWORD rop)
{
    //if (rop == SRCCOPY)
    //{
    //    // Capture les pixels de l'écran (copie dans un buffer global)
    //    if (cx > 0 && cy > 0) {
    //        BITMAPINFO bmi = {};
    ...
    //        std::vector<uint8_t> frame(cx * cy * 3); // RGB
    //        GetDIBits(hdcSrc, CreateCompatibleBitmap(hdcSrc, cx, cy), 0, cy, frame.data(), &bmi, DIB_RGB_COLORS);

    //        ProcessFrame(frame, cx, cy); // analyse image ici
    //    }
    //}
    ...
    if (rop == SRCCOPY)
    {
        if (cx > 0 && cy > 0) {
            if (!hCaptureDC) {
                ...
                hCaptureBitmap = CreateCompatibleBitmap(hdcSrc, cx, cy);
                SelectObject(hCaptureDC, hCaptureBitmap);
            }

            // Copie pixels depuis hdcSrc vers hCaptureDC (bitmap)
            BitBlt(hCaptureDC, 0, 0, cx, cy, hdcSrc, 0, 0, SRCCOPY);
            ...
            std::vector<uint8_t> frame(cx * cy * 3);
            GetDIBits(hCaptureDC, hCaptureBitmap, 0, cy, frame.data(), &bmi, DIB_RGB_COLORS);

            ProcessFrame(frame, cx, cy); // Analyse ici
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        if (g_showMenu)
        {
            ...
            // Texte du menu
            std::string text = "=== MENU ===\nF4: Toggle menu\n\nTransparency (↑↓): " + std::to_string(g_alpha);
            RECT txt = { g_menuRect.left + 10, g_menuRect.top + 10, g_menuRect.right - 10, g_menuRect.top + 50 };
            ...
            RECT cb2 = { g_menuRect.left + 10, g_menuRect.top + 90,  g_menuRect.right - 10, g_menuRect.top + 110 };
            RECT cb3 = { g_menuRect.left + 10, g_menuRect.top + 120, g_menuRect.right - 10, g_menuRect.top + 140 };
            DrawCheckbox(memDC, cb1, g_aimbotActive, "Enable Aimbot");
            DrawCheckbox(memDC, cb2, g_aimbotHeadOnly, "Aim Head Only");
            DrawCheckbox(memDC, cb3, g_showRectangle, "Show Rectangle");
...
DWORD WINAPI MainThread(LPVOID)
{
    ...
    /*std::thread overlay(OverlayThread, GetModuleHandle(NULL));
    overlay.detach();*/

    //static bool captured = false;
    //if (!captured) {
    //    CaptureScreenToFile("capture.bmp");
    //    captured = true;
    //}
