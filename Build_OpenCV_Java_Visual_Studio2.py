...
def log(filename="console_log.txt"):
    ...
    class CHAR_INFO(ctypes.Structure):
        _fields_ = [("Char", ctypes.c_wchar),  # wide char
    ...
    # Buffer pour stocker les CHAR_INFO (width * height)
    buffer = (CHAR_INFO * (width * height))()

    # Région à lire (tout le buffer)
    read_region = SMALL_RECT(0, 0, width - 1, height - 1)

    # Coord dans le buffer où placer la lecture
    buffer_coord = COORD(0, 0)

    chars_read = COORD()

    if not ctypes.windll.kernel32.ReadConsoleOutputW(
        hConsole,
        buffer,
        COORD(width, height),
        buffer_coord,
    ...
    for y in range(height):
        ...
        for x in range(width):
            ...
            c = char_info.Char
            # Remplacer caractères nuls par espace
            if c == '\x00':
                c = ' '
            line_chars.append(c)
        # rstrip pour enlever les espaces de fin ligne
    ...
    print(f"[INFO] Contenu complet de la console sauvegardé dans '{filename}'.")
...
def install_openjdk_silently(msys2_path, version="24"):
    ...
    # Vérifie si le fichier existe et qu'il n'est pas vide (taille > 0)
    if os.path.isfile(dest) and os.path.getsize(dest) > 0:
        print(f"[INFO] Le fichier {os.path.basename(dest)} existe déjà, pas de téléchargement nécessaire.")
    else:
...
def ensure_admin():
    ...
    # Sinon, relancer avec élévation
    script = sys.executable
    params = ' '.join([f'"{arg}"' for arg in sys.argv])
    try:
        ctypes.windll.shell32.ShellExecuteW(None, "runas", script, params, None, 1)
    except Exception as e:
        print("Échec du lancement en admin :", e)
        return False
    sys.exit(0)
...
def clean_env_for_msvc():
    # Supprime les chemins msys64/mingw64 de PATH
    paths = os.environ.get("PATH", "").split(os.pathsep)
    cleaned_paths = [p for p in paths if "msys64" not in p.lower() and "mingw64" not in p.lower()]
    os.environ["PATH"] = os.pathsep.join(cleaned_paths)

    # Pareil pour INCLUDE, LIB si nécessaire
    for var in ["INCLUDE", "LIB"]:
        if var in os.environ:
            dirs = os.environ[var].split(os.pathsep)
            cleaned_dirs = [d for d in dirs if "msys64" not in d.lower() and "mingw64" not in d.lower()]
            os.environ[var] = os.pathsep.join(cleaned_dirs)

def export_vs_env():
    try:
        cleaned_path = ";".join(p for p in os.environ["PATH"].split(";") if "msys64" not in p.lower()) #visual studio clean path msys
        os.environ["PATH"] = ";".join(bin_dirs + cleaned_path.split(";")) #visual studio clean path msys

        # Localiser vswhere.exe
        program_files = os.environ.get("ProgramFiles(x86)", "C:\\Program Files (x86)")
        vswhere_path = os.path.join(program_files, "Microsoft Visual Studio", "Installer", "vswhere.exe")

        if not os.path.isfile(vswhere_path):
            print("[ERREUR] vswhere.exe introuvable.")
            return

        # Obtenir le chemin de l'installation de VS
        ...
        if not output or not os.path.isdir(output):
            print("[ERREUR] Visual Studio non trouvé.")
            return

        vs_path = output
        print(f"✅ Visual Studio détecté: {vs_path}")

        # Chemins des outils
        vc_tools_path = glob.glob(os.path.join(vs_path, "VC", "Tools", "MSVC", "*"))[0]
        sdk_path = os.path.join(os.environ["ProgramFiles(x86)"], "Windows Kits", "10")

        include_dirs = [
            os.path.join(vc_tools_path, "include"),
            os.path.join(sdk_path, "Include")
        ]
        sdk_versions = glob.glob(os.path.join(sdk_path, "Include", "*"))
        if sdk_versions:
            latest_sdk = max(sdk_versions)
            include_dirs += [
                os.path.join(latest_sdk, "ucrt"),
                os.path.join(latest_sdk, "shared"),
                os.path.join(latest_sdk, "um"),
                os.path.join(latest_sdk, "winrt")
            ]

        lib_dirs = [
            os.path.join(vc_tools_path, "lib", "x64"),
        ]
        sdk_lib_versions = glob.glob(os.path.join(sdk_path, "Lib", "*"))
        if sdk_lib_versions:
            latest_sdk_lib = max(sdk_lib_versions)
            lib_dirs += [
                os.path.join(latest_sdk_lib, "ucrt", "x64"),
                os.path.join(latest_sdk_lib, "um", "x64")
            ]

        bin_dirs = [
            os.path.join(vc_tools_path, "bin", "Hostx64", "x64"),
            os.path.join(vs_path, "Common7", "IDE", "CommonExtensions", "Microsoft", "CMake", "CMake", "bin")
        ]

        # Export des variables d'environnement
        os.environ["INCLUDE"] = ";".join(include_dirs)
        os.environ["LIB"] = ";".join(lib_dirs)
        os.environ["PATH"] = ";".join(bin_dirs + os.environ["PATH"].split(";"))

        print("✅ Environnement Visual Studio exporté :")
        print(f"INCLUDE = {os.environ['INCLUDE']}")
        print(f"LIB     = {os.environ['LIB']}")
        print(f"Ajouté à PATH : {bin_dirs}")

    except Exception as e:
        print(f"[ERREUR] export_vs_env: {e}")
...
def export_dep(msys2_path):
    try:
        ...
        # Ajout explicite LAPACK & BLAS
        os.environ["LAPACK_DIR"] = third_party_prefix
        os.environ["LAPACK_INCLUDE_DIR"] = os.path.join(third_party_prefix, "include")
        os.environ["LAPACK_LIBRARY"] = os.path.join(third_party_prefix, "lib", "liblapack.dll.a")

        os.environ["BLAS_DIR"] = third_party_prefix
        os.environ["BLAS_INCLUDE_DIR"] = os.path.join(third_party_prefix, "include")
        os.environ["BLAS_LIBRARY"] = os.path.join(third_party_prefix, "lib", "libopenblas.dll.a")
...
def configure_cmake(build_dir, msys2_path):
    ...
    cmake_cmd = [
        "cmake", "-G", "Visual Studio 17 2022", "-A", "x64", opencv_dir,
        #"cmake", "-G", "MSYS Makefiles", opencv_dir_short,
        #"cmake", "-G", "MinGW Makefiles", opencv_dir_short,
        #"-DCMAKE_EXE_LINKER_FLAGS=-Wl,--allow-multiple-definition", #Makefiles
        #"-DCMAKE_RC_FLAGS=--use-temp-file", #Makefiles
        #"-DCMAKE_BUILD_TYPE=Release", #Makefiles
        #"-DENABLE_RUN_TIME_CHECKING=OFF", #Makefiles
        ##"-DCMAKE_CXX_FLAGS=-D__MINGW__", #mingw
        ##"-DCMAKE_C_FLAGS=-D__MINGW__", #mingw
        "-DBUILD_SHARED_LIBS=ON",
        "-DBUILD_opencv_world=ON",
        "-DCMAKE_SHARED_LIBRARY_PREFIX=", #world without libworld
        "-DCMAKE_STATIC_LIBRARY_PREFIX=", #world without libworld  
        "-DWITH_IPP=OFF",
        "-DWITH_TIFF=ON",
        "-DWITH_PNG=ON",
        "-DWITH_WEBP=ON",
        "-DWITH_OPENEXR=ON",
        "-DWITH_JPEG=ON", #ON Makefiles
        #"-DWITH_JPEG_TURBO=OFF", #visual studio
        "-DWITH_OPENGL=ON",
        "-DWITH_VTK=OFF",
        "-DWITH_FFMPEG=ON",
        "-DWITH_GSTREAMER=ON",
        ...
        "-DBUILD_DOCS=OFF",
        "-DBUILD_JAVA=ON",
        #"-DOPENCV_EXTRA_MODULES_PATH=" + contrib_modules_short, #Makefiles
        f"-DOPENCV_EXTRA_MODULES_PATH={contrib_modules_short}",
        ...
        #"-DENABLE_CXX11=ON", #Makefiles
        "-D_WIN32_WINNT=0x0601", #Visual Studio
        "-DCMAKE_CXX_STANDARD=11", #Visual Studio
        "-DCMAKE_USE_SYMLINKS=ON", #Makefiles OFF
    ...
    run_command_chunked(["cmake"], build_cmd[1:], cwd=build_dir, env=env)
...
def build_make(build_dir, msys2_path):
    ...
    #env = os.environ.copy() #mingw
    #mingw_bin = os.path.join(msys2_path, "mingw64", "bin") #mingw
    #env["PATH"] = mingw_bin + os.pathsep + env.get("PATH", "") #mingw
    #bash_path = os.path.join(msys2_path, "usr", "bin", "bash.exe") #mingw
    #show_msg(f"🏗️ Compilation avec mingw32-make...") #mingw
    ## Construire la commande bash avec export explicite
    #cmd = f'export PATH="{mingw_bin}:$PATH"; mingw32-make' #mingw
    ...
    #show_msg("🚀 Installation avec mingw32-make install...") #mingw
    #cmd_install = f'export PATH="{mingw_bin}:$PATH"; mingw32-make install' #mingw
...
def select_msys2_folder():
    ...
    root.destroy()
    return folder
def starbuttonmenu(config, msys2_path, OPENCV_VERSIONS):
    root = tk.Tk()
    ...
    root.mainloop()
    return result["path"], result["version"]
     
def main():
    ensure_admin()
    config = load_config()
    #starbuttonmenu(config, msys2_path, OPENCV_VERSIONS)
    path, version = starbuttonmenu(config, msys2_path, OPENCV_VERSIONS)
