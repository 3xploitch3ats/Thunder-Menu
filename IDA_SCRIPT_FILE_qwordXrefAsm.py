                                    ...
                                    try:
                                        ...
                                        if pseudocode:
                                            ...
                                            if pseudocode_lines:
                                                ...
                                                file.write("\n")
                                                # Récupérer la première fonction ASM de l'adresse Xref
                                                disasm = idaapi.generate_disasm_line(xref_address, 0)
                                                filtered_disasm = "".join(c for c in disasm if c.isprintable())
                                                if filtered_disasm:
                                                    file.write("First ASM Function: {}\n".format(filtered_disasm))
                                                    file.write("\n")
                                    except idaapi.DecompilationFailure:
                                        file.write("Xref Address: {}\n".format(hex(xref_address)))
                                        file.write("Xref Address - Imagebase: {}\n".format(hex(xref_address - imagebase)))
                                        file.write("Function: Decompilation failed\n")
                                        file.write("\n")
